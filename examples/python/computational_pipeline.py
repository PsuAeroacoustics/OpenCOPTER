...
if __name__ == "__main__":
    with open('geometry.json') as geom_data:
        geometry = json.load(geom_data)
    with open('parameters.json') as parameter:
        param = json.load(parameter)

    num_rotors = geometry["number of rotors"]
    num_blades = geometry["number of blades"]
    R = geometry["radius"]

    theta_75 = geometry["theta_75"]*(math.pi/180.0)
    AR = geometry["AR"]

    iterations = param["computational parameters"][0]["iterations"]
    print("iterations = ", iterations)
    wake_history_length = param["computational parameters"][0]["wake_hist_length"]

    requested_elements = param["computational parameters"][0]["requested elements"]
    V_inf = param["flight conditions"][0]["V_inf"]
    aoa = param["flight conditions"][0]["aoa"]*(math.pi/180.0)
    density = param["flight conditions"][0]["density"]
    omega =  param["flight conditions"][0]["omega"]
    sos = param["flight conditions"][0]["sos"]
    theta_tw_1 = geometry["theta_tw_1"]*(math.pi/180.0)

    d_psi = param["computational parameters"][0]["d_psi"]
    dt = d_psi*(math.pi/180)/math.fabs(omega)
    
    shed_history_angle = param["computational parameters"][0]["shed_history_angle"]
    shed_history = round(shed_history_angle/d_psi)

    d_azimuth = 2.0*math.pi/num_blades

    #Creat the spanwise distribution for our blades
    r = generate_radius_points(requested_elements)
    elements = len(r)
    print("requested elements:", requested_elements,"actual elements:", elements)
    c = (1.0/AR)*np.ones(elements)
    alpha_0 = np.zeros(elements)
    twist = [(_r - 0.75)*theta_tw_1 for _r in r]
    Cl_alpha = 2.0*math.pi*np.ones(elements)
