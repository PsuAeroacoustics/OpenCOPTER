...
if __name__ == "__main__":
	...
	def build_rotor(r_idx, parent_frame):
		...
		rotor_fixed_frame = Frame(Vec3([0, 0, 1]), 0, Vec3([0, 0, 0]), parent_frame, f'rotor_{r_idx}_fixed', FrameType_connection())
		rotor_frame = Frame(Vec3([0, 0, 1]), 0, Vec3([0, 0, 0]), rotor_fixed_frame, f'rotor_{r_idx}', FrameType_rotor())

		rotor_fixed_frame.children = [rotor_frame]
	...
	aircraft.root_frame.children = [r.frame.parent for r in aircraft.rotors]
	...
	for iteration in range(iterations):
		...
		for i in range(0, num_rotors):
			#print(f'azimuth: {ac_input_state.rotor_inputs[i].azimuth}')
			aircraft.rotors[i].frame.set_rotation(Vec3([0, 0, 1]), ac_input_state.rotor_inputs[i].azimuth)
