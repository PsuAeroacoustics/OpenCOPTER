...
extern(C++) struct TipVortexT(ArrayContainer AC) {
	mixin ArrayDeclMixin!(AC, VortexFilamentT!(AC), "trailing_filaments");
...
struct WakeHistoryT(ArrayContainer AC) {
	...
	this(size_t num_rotors, size_t num_blades, size_t wake_history, size_t time_history, size_t radial_elements, size_t[] num_trailing_vortices, size_t[] shed_history, size_t[] shed_release, double _a1 = 6.5e-5, bool hybrid = false) {
		...
		foreach(ref wake; history) {
			wake = WakeT!AC(num_rotors, num_blades, wake_history, radial_elements, num_trailing_vortices, shed_history, shed_release);
		}
	}
		...
		for(size_t i = history.length - 1; i > 0; i--) {
			foreach(rotor_idx, ref rotor_wake; history[i - 1].rotor_wakes) {
				foreach(blade_idx, ref blade_trailers; rotor_wake.blade_trailers) {
					...
					foreach(f_idx, ref trailing_filament; blade_trailers.trailing_filaments){
...
void fill_wake_xyz_rotor_frame(RG, VF)(auto ref RG rotor, auto ref VF filament, double[] x, double[] y, double[] z) {
	foreach(c_idx, ref chunk; filament.chunks) {
		...
		auto xyz_tpp = rotor.frame.parent.inverse_global_matrix * xyz_chunk; //back corrected!!
...
InducedVelocities compute_filament_induced_velocities(FC, BWI)(auto ref FC chunks, immutable Chunk x, immutable Chunk y, immutable Chunk z, size_t chunk_offset, auto ref BWI BWIinputs, immutable Chunk x_old, immutable Chunk y_old, immutable Chunk z_old, size_t blade_chunk_idx = 0, bool save_BWIinputs = false) {
	...
	//		  Check if we can store the required values in FC "chunks"
	...
	foreach(i_c_idx, ref chunk_i; chunks[chunk_offset..$]) {
		...
		foreach(n_idx; 0..Chunk.length) {
			...
			if(n_idx == 0){
				...
				dx_p = x_p[n_idx] - x_p[n_idx-1];
				dy_p = y_p[n_idx] - y_p[n_idx-1];
				dz_p = z_p[n_idx] - z_p[n_idx-1];
			}
			...
			if(save_BWIinputs){
				if((n_idx == 0) && (blade_chunk_idx == 0)) {
					...
					foreach(bwi_idx;0..Chunk.length){
						if(BWIinputs[i_c_idx].miss_dist[bwi_idx] > miss_dist[bwi_idx]){
							BWIinputs[i_c_idx].miss_dist[bwi_idx] = miss_dist[bwi_idx];
	...
	if(single_rotor) {
		...
		foreach(i_rotor_idx, ref i_rotor; ac_state.rotor_states) {
			foreach(i_blade_idx; 0..ac_state.rotor_states[i_rotor_idx].blade_states.length) {
				...
				//writeln("in compute_wake_ind_vel_tip_only");
				if(!tip_only) {
		...
		//writeln("in compute_wake_ind_vel_shed_only");
		if(!shed_only) {
			foreach(i_rotor_idx, ref i_rotor; ac_state.rotor_states) {
	...
	if((isInstanceOf!(WakeT, W) || (isPointer!W && isInstanceOf!(WakeT, PointerTarget!W))) && is_aircraft_state!AS)
{
	...
	foreach(rotor_idx, ref i_rotor; ac_state.rotor_states) {
		foreach(blade_idx; 0..ac_state.rotor_states[rotor_idx].blade_states.length){
			...
			BWIinputsChunk[] dummy_chunks;
			auto ind_vel = compute_filament_induced_velocities(ac_state.rotor_states[rotor_idx].blade_states[blade_idx].chunks, x, y, z, chunk_offset, dummy_chunks, x_old, y_old, z_old, 0, false);
			ret.v_x[] += ind_vel.v_x[];
			ret.v_y[] += ind_vel.v_y[];
			ret.v_z[] += ind_vel.v_z[];
			...
			foreach(fil_idx, ref shed_filament; wake.rotor_wakes[rotor_idx].shed_vortices[blade_idx].shed_filaments){
				auto ind_vel_shed = compute_filament_induced_velocities(shed_filament.chunks, x, y, z, chunk_offset, dummy_chunks, x_old, y_old, z_old, 0, false);
...
void update_wake(ArrayContainer AC = ArrayContainer.None)(ref AircraftT!AC ac, ref AircraftStateT!AC ac_state, ref AircraftInputStateT!AC ac_input_state, ref WakeHistoryT!AC wake_history, immutable Atmosphere atmo, size_t time_step, double dt) {
	...
	foreach(rotor_idx, ref rotor; ac_state.rotor_states) {
		...
		foreach(blade_idx, ref blade; rotor.blade_states) {
			...
			immutable num_blade_elements = num_chunks*chunk_size;
			...
			if (num_trailers == 2){
				...
				trailing_gamma[0] = -max_gamma;
				trailing_gamma[1] = max_gamma;
			...
			}else if((num_blade_elements%num_trailers == 1) && (num_blade_elements%(num_trailers-1) == 0)){
				double[] seg_gamma;
				auto num_segs = num_trailers-1;
				seg_gamma = new double[num_segs];
				auto num_elem_in_one_seg = num_blade_elements/num_segs;
				foreach(i;0..num_segs){
					auto element_idx = i*num_elem_in_one_seg;
					auto blade_chunk_idx = element_idx%chunk_size;
					auto c_idx = element_idx - blade_chunk_idx*chunk_size;
					trailing_rad_location[i] = ac.rotors[rotor_idx].blades[blade_idx].chunks[blade_chunk_idx].r[c_idx];
					trailing_xi_location[i] = ac.rotors[rotor_idx].blades[blade_idx].chunks[blade_chunk_idx].xi[c_idx];
					auto gamma_tmp = 0.0;
					foreach(j;0..num_elem_in_one_seg){
						auto spanwise_element_number = i*num_elem_in_one_seg + j;
						immutable ch_idx_gamma = spanwise_element_number%chunk_size;
						immutable c_idx_gamma = spanwise_element_number - ch_idx_gamma*chunk_size;
						gamma_tmp += ac_state.rotor_states[rotor_idx].blade_states[blade_idx].chunks[ch_idx_gamma].gamma[c_idx_gamma];
					}
					seg_gamma[i] = gamma_tmp;
				}
				trailing_rad_location[num_segs] = 1.0;
				trailing_xi_location[num_segs] = ac.rotors[rotor_idx].blades[blade_idx].chunks[$-1].xi[$-1];
				foreach(i;0..num_trailers){
					if(i==0){
						trailing_gamma[i] = -seg_gamma[i];
			...
			foreach(trailing_idx, ref trailing_filament ; current_blade_trailers.trailing_filaments){
				...
				trailing_filament.chunks[0].gamma[0] = trailing_gamma[trailing_idx];
				...
				foreach(c_idx, ref chunk; wake_history.history[1].rotor_wakes[rotor_idx].blade_trailers[blade_idx].trailing_filaments[trailing_idx].chunks) {
					...
					if(c_idx > 0) {
						trailing_filament.chunks[c_idx].d_volume[0] = wake_history.history[1].rotor_wakes[rotor_idx].blade_trailers[blade_idx].trailing_filaments[trailing_idx].chunks[c_idx - 1].volume[$ - 1] - trailing_filament.chunks[c_idx].volume[0];
					}

					// Only update index 0 if this is not the last chunk. Index 0 of the first chunk is
					// the most recent and is updated later.
					if(c_idx != wake_history.history[1].rotor_wakes[rotor_idx].blade_trailers[blade_idx].trailing_filaments[trailing_idx].chunks.length - 1) {
