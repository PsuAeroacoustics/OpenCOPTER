...
extern (C++) struct WingFilamentChunk {
    ...
    Chunk normalized_gamma; // Non-dimensionalized vortex circulation (gamma/(V*delta_y))
    Chunk A_kl; // d_gamma/dy|_x
    Chunk circulation_multpld_by_v_square;
...
struct VortexLatticeT(ArrayContainer AC) {
    ...
    void compute_bound_circulation(WLS,WPG)(auto ref WLS wing_lift_surface, auto ref WPG wing_part, size_t wp_idx, size_t span_chunk_idx, size_t chord_node_idx, immutable Chunk u){
        double gamma = 0.0;
    ...
    void compute_dCl(WLS,WPS)(auto ref WLS wing_lift_surface, auto ref WPS wing_part_state, size_t wp_idx, size_t span_chunk_idx){
        ...
        foreach(c1; 0..chunk_size){
            foreach(n_idx; 0..num_chord_pt){
                ...
                immutable dCl_inter = PI*(wing_lift_surface.wing_part_lift_surf[wp_idx].spanwise_filaments[n_idx].chunks[span_chunk_idx].normalized_gamma[c1])*sin(theta_n)/num_chord_pt;
                immutable dCl_dim_inter = PI*(wing_lift_surface.wing_part_lift_surf[wp_idx].spanwise_filaments[n_idx].chunks[span_chunk_idx].circulation_multpld_by_v_square[c1])*sin(theta_n)/num_chord_pt;
                dCl += dCl_inter;
                dCl_dim += dCl_dim_inter;
            }
            wing_part_state.chunks[span_chunk_idx].dC_L[c1]= dCl;
            wing_part_state.chunks[span_chunk_idx].dCL_dim[c1] = dCl_dim;
            dCl = 0.0;            
        }
    }    
}
