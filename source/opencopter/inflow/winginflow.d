...
class WingInflowT(ArrayContainer AC = ArrayContainer.none) : InflowT!AC {
    ...
    this(WG* _wing, WIS* _wing_input, WLS* _wing_lift_surf){
        ...
        //local_frame.local_matrix = _wing.frame.local_matrix;
    ...
    void update(AircraftStateT!AC ac_state, WakeT!AC wake, double dt){
        ...
        foreach(wp_idx, wing_part; wing.wing_parts){
            ...
            foreach(ch_idx,ctrl_chunk; wing_part.ctrl_chunks){
                if (!wing_only_test){
                    ...
                    auto xyz_global = local_frame.global_matrix * ctrl_xyz;
                    ...
                    if(calc_wake_ind_vel){
                        ...
                        foreach(rotor_idx, ref rotor; ac_state.rotor_states) {
                            ...
                            auto xyz_tpp = rotor.inflow_model.frame.inverse_global_matrix * xyz_global;                    
                            ...
                            combined_inflow += rotor.inflow_model.frame.global_matrix * local_inflow_due_to_rotor;
                        }
                    }
                    ...
                    foreach(if_idx, ref wing; ac_state.wing_states){
                        if(wing.inflow_model != this) {
                            auto xyz_tpp = wing.inflow_model.frame.inverse_global_matrix * xyz_global;
                            ...
                            combined_inflow += wing.inflow_model.frame.global_matrix * local_inflow_due_to_wing;
        ...
        foreach(w_idx, ref ws; ac_state.wing_states) {
            if (ws.inflow_model.frame == this.frame) {
                compute_wing_C_L(wing_lift_surf, wing, ws);
            }
        }
        ...
        foreach(wp_idx, wing_part; wing.wing_parts){
            foreach(chord_idx;0..num_chord_pt){
                foreach(ch_idx; 0..num_span_chunks){
                    ...
                    wing_state.wing_part_states[wp_idx].circulation_model.compute_d_gamma_coefficients(wing_lift_surf, wing_state.wing_part_states[wp_idx], wp_idx, ch_idx, chord_idx, u_inf);
                }
            }
