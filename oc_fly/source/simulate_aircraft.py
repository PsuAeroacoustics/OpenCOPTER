...
def simulate_aircraft(log_file, vehicle: SimulatedVehicle, atmo, elements, write_wake, vtu_output_path, wopwop_output_path, do_compute, flight_condition, computational_parameters, observer, acoustics, wake_lengths, results, wopwop_motion):
	...
	for rotor in vehicle.aircraft.rotors:
		for blade in rotor.blades:
			wopwop_input_files_generator.write_wopwop_geometry(naca0012_xsection, wopwop_data_path, rotor, blade)
	...
	moment_taus[0, 0] = 28.0
	moment_taus[0, 1] = 28.0
	...
	span_element_aoa_eff = np.zeros((num_rotors, len(target_span_elements), int(iter_per_rev)))
	span_element_aoa = np.zeros((num_rotors, len(target_span_elements), int(iter_per_rev)))
	span_element_up = np.zeros((num_rotors, len(target_span_elements), int(iter_per_rev)))
	span_element_inflow_angle = np.zeros((num_rotors, len(target_span_elements), int(iter_per_rev)))
	span_element_theta = np.zeros((num_rotors, len(target_span_elements), int(iter_per_rev)))
	span_element_gamma = np.zeros((num_rotors, len(target_span_elements), int(iter_per_rev)))
	...
	if do_compute:
		...
		while not sim_done:
			...
			for r_idx, rotor in enumerate(vehicle.aircraft.rotors):
				_ = basic_single_rotor_dynamics(vehicle.input_state.rotor_inputs[r_idx], dt)

				for motion_lambda in vehicle.motion_lambdas[r_idx]:
					motion_lambda(vehicle.input_state.rotor_inputs[r_idx].azimuth)
			...
			if converged:
				if write_wake and (converged_revolutions >= (post_conv_revolutions - 1)):
					for r_idx in range(num_rotors):
						write_rotor_vtu(f"{vtu_output_path}/rotor", acoustic_iteration, r_idx, vtk_rotors[r_idx], vehicle.ac_state.rotor_states[r_idx], vehicle.input_state.rotor_inputs[r_idx], rotor)
				...
				for rotor_idx, rotor in enumerate(vehicle.ac_state.rotor_states):
					...
					if start_recording and not done_recording:
						for t_idx in range(len(target_span_elements)):
							dC_L = vehicle.ac_state.rotor_states[r_idx].blade_states[2].chunks[target_span_chunk_index[r_idx][t_idx]].dC_L[target_span_element_index[r_idx][t_idx]]
							aoa_eff = vehicle.ac_state.rotor_states[r_idx].blade_states[2].chunks[target_span_chunk_index[r_idx][t_idx]].aoa_eff[target_span_element_index[r_idx][t_idx]]
							aoa = vehicle.ac_state.rotor_states[r_idx].blade_states[2].chunks[target_span_chunk_index[r_idx][t_idx]].aoa[target_span_element_index[r_idx][t_idx]]
							u_p = vehicle.ac_state.rotor_states[r_idx].blade_states[2].chunks[target_span_chunk_index[r_idx][t_idx]].u_p[target_span_element_index[r_idx][t_idx]]
							inflow_angle = vehicle.ac_state.rotor_states[r_idx].blade_states[2].chunks[target_span_chunk_index[r_idx][t_idx]].inflow_angle[target_span_element_index[r_idx][t_idx]]
							theta = vehicle.ac_state.rotor_states[r_idx].blade_states[2].chunks[target_span_chunk_index[r_idx][t_idx]].theta[target_span_element_index[r_idx][t_idx]]
							gamma = vehicle.ac_state.rotor_states[r_idx].blade_states[2].chunks[target_span_chunk_index[r_idx][t_idx]].gamma[target_span_element_index[r_idx][t_idx]]

							span_element_loading[rotor_idx, t_idx, spanwise_element_iteration] = dC_L*atmo.density*math.pi*vehicle.aircraft.rotors[rotor_idx].radius**3.0*abs(omegas[rotor_idx])**2.0/(0.5*atmo.density*flight_condition["sos"]**2.0*vehicle.aircraft.rotors[0].blades[2].average_chord)
					...
					for t_idx in range(len(target_y_slices)):
						for r_idx in range(num_rotors):
							if wake_element_found[r_idx][t_idx]:
								...
								wake_element_trajectory[r_idx, t_idx, 0, wake_element_index[r_idx, t_idx]] = temp_wake_array_x[wake_element_index[r_idx, t_idx]]/vehicle.aircraft.rotors[r_idx].radius
								wake_element_trajectory[r_idx, t_idx, 1, wake_element_index[r_idx, t_idx]] = temp_wake_array_z[wake_element_index[r_idx, t_idx]]/vehicle.aircraft.rotors[r_idx].radius
